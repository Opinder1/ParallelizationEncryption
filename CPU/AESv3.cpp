...
namespace aes::v3
{
	...
	__forceinline void SubBytesShift(const unsigned char block[16], unsigned char temp[16])
	{
		temp[0] = sbox[block[0]];
		temp[1] = sbox[block[5]];
	...
	__forceinline void InvSubBytesShift(const unsigned char block[16], unsigned char temp[16])
	{
	...
	template<class T>
	__forceinline void Shift1(T& a, T& b, T& c, T& d)
	{
		T temp = a;
		a = b;
		b = c;
	...
	__forceinline void KeyExpansion(const unsigned char* input, unsigned char* output)
	{
		// First
