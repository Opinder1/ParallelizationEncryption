...
__constant const uchar sbox[16 * 16] = {
...
__constant const uchar inv_sbox[16 * 16] = {
...
__constant const uchar rcon[11] = { 0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36 };

uchar16 SubBytesShift(uchar16 block)
{
	uchar16 temp;
...
uchar16 InvSubBytesShift(uchar16 block)
{
	uchar16 temp;
...
uchar16 EncryptBlockECB(__global const uchar16* subkeys, uint rounds, uchar16 block)
{
	block ^= subkeys[0];

	for (size_t i = 1; i < rounds; i++)
	{
		uchar16 temp = SubBytesShift(block);
	...
	block = temp ^ subkeys[rounds];

	return block;
}

uchar16 DecryptBlockECB(__global const uchar16* subkeys, uint rounds, uchar16 block)
{
	block ^= subkeys[rounds];

	for (size_t i = rounds - 1; i > 0; i--)
	{
		uchar16 temp = InvSubBytesShift(block);
	...
	block = temp ^ subkeys[0];

	return block;
}

__kernel void EncryptBlockCTR(__global const uchar16* subkeys, uint rounds, __global uchar16* blocks)
{
	size_t index = get_global_id(0);

	uchar16 block = blocks[index];
	uchar16 index_block = 0;
...
__kernel void DecryptBlockCTR(__global const uchar16* subkeys, uint rounds, __global uchar16* blocks)
{
	size_t index = get_global_id(0);
