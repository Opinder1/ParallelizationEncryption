...
void SubBytesShift(__global unsigned char block[16], unsigned char temp[16])
{
	temp[0] = sbox[block[0]];
	temp[1] = sbox[block[5]];
...
void InvSubBytesShift(__global unsigned char block[16], unsigned char temp[16])
{
...
__kernel void encrypt(__global unsigned char* subkeys, unsigned int rounds, __global unsigned char* block)
{
	block += get_global_id(0) * 16;
...
__kernel void decrypt(__global const unsigned char* subkeys, unsigned int rounds, __global unsigned char* block)
{
	block += get_global_id(0) * 16;
